...
RSpec.describe Event, type: :model do
  ...
  context 'When save fails' do
    ...
    it 'Should not create events when start_at is after end_at' do
      start_at = 4.days.from_now
      end_at = start_at - 2.hours
      ...
      expect(event.save).to be false
      expect(event.errors.messages[:end_at].first).to eq('A data de término do evento deve ser maior que a data de início')
    end
  end
